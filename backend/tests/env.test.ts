...
const valid = { NODE_ENV: "test", APP_URL: "http://localhost:3000", DATABASE_URL: "mongodb://localhost:27018/test?replicaSet=rs0", SESSION_SECRET: "x".repeat(32), OTP_HMAC_SECRET: "o".repeat(32), FIELD_ENCRYPTION_KEY_BASE64: Buffer.alloc(32).toString("base64"), MPESA_CONSUMER_KEY: "key", MPESA_CONSUMER_SECRET: "secret", MPESA_SHORTCODE: "174379", MPESA_PASSKEY: "passkey", MPESA_CALLBACK_URL: "https://example.com/api/webhooks/mpesa" };
describe("environment validation", () => {
  ...
  it("rejects a missing OTP HMAC secret", () => expect(() => parseServerEnv({ ...valid, OTP_HMAC_SECRET: undefined } as NodeJS.ProcessEnv)).toThrow());
