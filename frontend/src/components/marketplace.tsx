...
export function Marketplace({ initialListings, initialTowns, stats, listPropertyHref = "/register?role=landlord&returnTo=%2Fdashboard%2Flandlord%23new-vacancy" }: { initialListings: ListingCard[]; initialTowns: string[]; stats: MarketplaceStats; listPropertyHref?: string }) {
  ...
  const [modal, setModal] = useState<"safety" | null>(null);
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(({ coords }) => setDetectedCoordinates({
      latitude: Number(coords.latitude.toFixed(3)),
      longitude: Number(coords.longitude.toFixed(3))
    }), () => undefined, { enableHighAccuracy: false, maximumAge: 300_000, timeout: 8_000 });
  }, []);
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    ...
    if (town === "all" && detectedTown) params.set("nearTown", detectedTown);
    else if (town === "all" && detectedCoordinates) {
      params.set("nearLat", String(detectedCoordinates.latitude));
      params.set("nearLng", String(detectedCoordinates.longitude));
    }
    ...
    fetch(`/api/listings?${params}`, { signal: controller.signal, cache: "no-store" })
      ...
      .then(payload => { setListings(listingCardsFromApi(payload.data)); setTowns(payload.towns); if (payload.detectedTown) setDetectedTown(payload.detectedTown); })
  ...
  }, [preferencesReady, town, minPrice, maxPrice, detectedTown, detectedCoordinates]);
  ...
  return <>
    <header className="topbar">
      ...
      <div className="header-actions"><Link className="text-button" href="/login">Sign in</Link><Link className="button small" href={listPropertyHref}>List a property</Link></div>
      <details ref={mobileMenuRef} className="mobile-navigation" onToggle={(event) => { if (event.currentTarget.open) menuOpenedAtScrollY.current = window.scrollY; }}>
        ...
        <div id="mobile-navigation" className="mobile-menu" role="navigation" aria-label="Mobile navigation" onClick={closeMobileMenu}>
          ...
          <Link className="button" href={listPropertyHref}>List a property</Link>
        </div>
      </details>
    </header>
    <main>
      <section className="hero"><div className="hero-copy"><div className="eyebrow">Verified rentals across Kenya</div><h1>Your next home is <em>closer</em> than you think.</h1><p>Discover genuinely vacant homes, compare rent and amenities, then unlock the exact location and landlord contact securely.</p><div className="trust-row"><span>✓ Verified listings</span><span>✓ M-Pesa payments</span><span>✓ No hidden agent fees</span></div></div>
        <form className="search-panel" onSubmit={search}>
          ...
          <label>House type<select value={type} onChange={(event) => setType(event.target.value)}><option value="all">Any house type</option>{UNIT_TYPES.map(value => <option key={value}>{value}</option>)}</select></label>
          ...
          {town === "all" && detectedTown && <p className="search-location-hint">Prioritizing homes in {detectedTown} from your approximate location.</p>}
        </form>
      </section>
